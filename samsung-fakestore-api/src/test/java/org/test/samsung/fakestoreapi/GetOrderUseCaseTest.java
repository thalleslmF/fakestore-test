...
class GetOrderUseCaseTest {
    ...
    @Test
    void testExecute_OrderWithNoProducts() {
        // Arrange
        var user = User.builder().id(1).name(new Name()).build();
        var order = Order.builder().id(1).date(Instant.now()).user(user).products(List.of()).build();
    ...
    @Test
    void testExecute_WithNullDateRange() {
        // Arrange
        var user = User.builder().id(1).name(new Name()).build();
        var product = Product.builder().id(1).quantity(1).build();
        var order = Order.builder().id(1).date(Instant.now()).user(user).products(List.of(product)).build();
        ...
        // Act
        List<Order> result = getOrderUseCase.execute(1, null, null, null);
    ...
    void testExecute_WithNullUserId() {
        ...
        // Act
        List<Order> result = getOrderUseCase.execute(null, Instant.now().minusSeconds(3600), Instant.now(), null);
